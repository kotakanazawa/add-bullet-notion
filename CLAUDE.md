...
NotionにバレットポイントとTODOチェックボックスを挿入するキーボードショートカットを追加するChrome拡張機能（Manifest V3）。ビルド不要、依存関係なし、テストなし。

## アーキテクチャ

- `manifest.json` — 拡張機能のマニフェスト。コンテンツスクリプトは `https://*.notion.so/*` に `document_end` で注入される。
- `content.js` — keydownイベントを監視し、`document.execCommand("insertText", ...)` でcontentEditable要素にテキストを挿入するコンテンツスクリプト。

## ショートカット

- `⌘ + Shift + 8` → `- `（バレットポイント）を挿入
- `⌘ + Shift + 9` → `[] `（TODOチェックボックス）を挿入

## 開発方法

`chrome://extensions/` でデベロッパーモードを有効にし、「パッケージ化されていない拡張機能を読み込む」でこのフォルダを選択する。ビルドやインストールコマンドは不要。ファイルを編集後、拡張機能をリロードすれば反映される。
...
- タスクが中規模以上、または曖昧さが残る場合：
  - 質問の回答が揃ったら、仕様を SPEC.md（docs/）にまとめる案を提示する
